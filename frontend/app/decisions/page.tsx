...
export default function DecisionsPage() {
  ...
  useEffect(() => { apiFetch("/api/workspaces").then(r => r.ok ? r.json() : Promise.reject()).then((items: Workspace[]) => { setWorkspaces(items); if (items[0]) setWorkspaceId(items[0].id); }).catch(() => setMessage("Sign in and create a workspace to manage engineering memory.")); }, []);
  useEffect(() => { if (!workspaceId) return; apiFetch(`/api/workspaces/${workspaceId}/memories`).then(r => r.ok ? r.json() : Promise.reject()).then(setMemories).catch(() => setMessage("Unable to load engineering memory.")); }, [workspaceId]);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); setMessage(""); try { const response = await apiFetch(`/api/workspaces/${workspaceId}/memories`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, source: "decision", content }) }); const data = await response.json(); if (!response.ok) throw new Error(data.message || "Unable to save decision"); setMemories(items => [data, ...items]); setTitle(""); setContent(""); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save decision"); } finally { setSaving(false); } }
